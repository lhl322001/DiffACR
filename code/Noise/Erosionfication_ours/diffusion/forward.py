...
class erode(ForwardProcessBase):
    
    def __init__(self,
                 image_size=(32,32),
                 erode_level=1,
                 num_timesteps=50,
                 erode_base_path=None,
                 random_erode=False,
                 single_erode=False,
                 batch_size=32,
    ...
    def reset_parameters(self, batch_size=-1):
        if batch_size != -1:
            self.batch_size = batch_size
        if self.random_erode:
            self.generate_erode_layer()



    @torch.no_grad()
    def generate_erode_layer(self):
        if not self.random_erode:
            rstate = np.random.get_state()
            np.random.seed(123321)
        if self.erode_level == 1:
            c = (0.1, 0.3, 3, 0.5, 5, 4, 0.8)
            erode_thres_start = 0.7
            erode_thres_end = 0.3
            mb_sigma_start = 0.5
            mb_sigma_end = 5.0
            br_coef_start = 0.95
            br_coef_end = 0.7
        elif self.erode_level == 2:
            c = (0.55, 0.3, 2.5, 0.85, 11, 12, 0.55) 
            erode_thres_start = 1.15
            erode_thres_end = 0.7
            mb_sigma_start = 0.05
            mb_sigma_end = 12
            br_coef_start = 0.95
            br_coef_end = 0.55
        elif self.erode_level == 3:
            ...
            br_coef_start = 0.95
            br_coef_end = 0.4
        elif self.erode_level == 4:
            c = (0.55, 0.3, 2.5, 0.55, 11, 20, 0.3) 
            erode_thres_start = 1.15
            erode_thres_end = 0.55
            mb_sigma_start = 0.05
            mb_sigma_end = 20
            br_coef_start = 0.95
            br_coef_end = 0.3
        ...
        self.erode_thres_list = torch.linspace(erode_thres_start, erode_thres_end, self.num_timesteps).tolist()
        ...
        if self.single_erode:
            ...
            for _ in range(self.batch_size):
                ...
                sb_list.append(cs)
            erode_layer_base = np.concatenate(sb_list, axis=2)
        else:
            erode_layer_base = np.random.normal(size=self.image_size, loc=c[0], scale=c[1])
            erode_layer_base = erode_layer_base[..., np.newaxis]
            erode_layer_base = clipped_zoom(erode_layer_base, c[2])
        
        vertical_erode = False
        if np.random.uniform() > 0.5:
            vertical_erode = True

        for i in range(self.num_timesteps):

            erode_layer = torch.Tensor(erode_layer_base).clone()
            erode_layer[erode_layer < self.erode_thres_list[i]] = 0
            erode_layer = torch.clip(erode_layer, 0, 1)
            erode_layer = erode_layer.permute((2, 0, 1)).unsqueeze(1)
            ...
            verode = F.conv2d(erode_layer, vertical_kernel, padding='same', groups=1)
            herode = F.conv2d(erode_layer, horizontal_kernel, padding='same', groups=1)
            if self.single_erode:
                vidx = torch.randperm(erode_layer.shape[0])
                vidx = vidx[:int(erode_layer.shape[0]/2)]
                erode_layer = herode
                erode_layer[vidx] = verode[vidx]
            elif vertical_erode:
                erode_layer = verode
            else:
                erode_layer = herode
            self.erode.append(erode_layer)
            self.erode_rot.append(torch.rot90(erode_layer, k=2, dims=[2,3]))
    ...
    def forward(self, x, i, og=None):
        ...
        if self.fix_brightness:
            erosion_img = torch.clip(og_r + self.erode[i].cuda() + self.erode_rot[i].cuda(), 0.0, 1.0)
        else:
            erosion_img = torch.clip(scaled_og + self.erode[i].cuda() + self.erode_rot[i].cuda(), 0.0, 1.0)
        return (erosion_img * 2.) - 1.
