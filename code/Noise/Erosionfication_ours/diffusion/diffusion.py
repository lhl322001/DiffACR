...
import imageio
from .forward import DeColorization, erode
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.erode_level = erode_level
        self.random_erode = random_erode
        self.batch_size = batch_size
        self.single_erode = single_erode
        ...
        if forward_process_type == 'Decolorization':
            self.forward_process = DeColorization(decolor_routine=decolor_routine,
                                                  ...
                                                  to_lab=self.to_lab,)
        elif forward_process_type == 'Erosion':
            if load_path is not None:
                erode_base_path = load_path.replace('model.pt', 'erode_base.npy')
                print(erode_base_path)
                load_erode_base = True
            else:
                erode_base_path = os.path.join(results_folder, 'erode_base.npy')
                load_erode_base = False
            self.forward_process = erode(image_size=self.image_size,
                                        erode_level=self.erode_level, 
                                        random_erode=self.random_erode,
                                        num_timesteps=self.num_timesteps,
                                        erode_base_path=erode_base_path,
                                        batch_size=self.batch_size,
                                        single_erode=self.single_erode,
                                        load_erode_base=load_erode_base,
...
class Trainer(object):
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                if True:
                    ...
                    for k, img in sample_dict.items():
                        img_scale = (img + 1) * 0.5
                        utils.save_image(img_scale, str(self.results_folder / f'sample-{k}-{milestone}.jpg'), nrow=6)
    ...
    def save_gif(self, X_0s, X_ts, extra_path, init_recon=None, og=None):
        ...
        for i in range(len(X_0s)):
            ...
            utils.save_image(x_0_grid, str(self.results_folder / f'sample-{i}-{extra_path}-x0.jpg'))
            self.add_title(str(self.results_folder / f'sample-{i}-{extra_path}-x0.jpg'), title_texts)
            ...
            utils.save_image(all_images_grid, str(self.results_folder / f'sample-{i}-{extra_path}-xt.jpg'))
            self.add_title(str(self.results_folder / f'sample-{i}-{extra_path}-xt.jpg'), title_texts)
    ...
    def save_og_test(self, og_dict, extra_path):
        ...
        for k, img in og_dict.items():
            ...
            utils.save_image(img_grid, str(self.results_folder / f'{k}-{extra_path}.jpg'))
            self.add_title(str(self.results_folder / f'{k}-{extra_path}.jpg'), '{k}')
            og_dict[k] = img_grid
