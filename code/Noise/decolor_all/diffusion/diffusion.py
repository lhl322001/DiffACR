...
import imageio
from .forward import DeColorization, Snow
        ...
        *,
        ...
        if forward_process_type == 'Decolorization':
            ...
            if load_path is not None:
                erosion_base_path = load_path.replace('model.pt', 'erosion_base.npy')
                print(erosion_base_path)
                load_erosion_base = True
            else:
                erosion_base_path = os.path.join(results_folder, 'erosion_base.npy')
                load_erosion_base = False
            self.forward_process = Snow(image_size=self.image_size,
                                        erode_level=self.erode_level, 
                                        random_snow=self.random_snow,
                                        num_timesteps=self.num_timesteps,
                                        snow_base_path=erosion_base_path,
                                        batch_size=self.batch_size,
                                        single_snow=self.single_snow,
                                        load_snow_base=load_erosion_base,
