...
class Trainer(object):
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                utils.save_image(direct_recons, str(self.results_folder / f'sample-direct_recons-{milestone}.jpg'), nrow=6)
        ...
        for j in range(int(6400/bs)):
            ...
            for i in range(all_images.shape[0]):
                utils.save_image((all_images[i] + 1) * 0.5,
                                 str(f'{out_folder}/' + f'sample-x0-{cnt}.jpg'))
    ...
    def paper_showing_diffusion_images_cover_page(self):
        ...
        for i in range(5):
            ...
            for k in range(Forward[0].shape[0]):
                ...
                utils.save_image(og_img[k], str(self.results_folder / f'og_img_{cnt}.jpg'), nrow=1)
                start = cv2.imread(f'{self.results_folder}/og_img_{cnt}.jpg')
                l.append(start)

                for j in range(len(Forward)):
                    x_t = Forward[j][k]
                    x_t = (x_t + 1) * 0.5
                    utils.save_image(x_t, str(self.results_folder / f'temp.jpg'), nrow=1)
                    x_t = cv2.imread(f'{self.results_folder}/temp.jpg')
                    if j in to_show:
                        l.append(x_t)
                ...
                utils.save_image(final_all[k], str(self.results_folder / f'final_{cnt}.jpg'), nrow=1)
                final = cv2.imread(f'{self.results_folder}/final_{cnt}.jpg')
                l.append(final)
        ...
        for i in range(50):
            ...
            for k in range(X_ts[0].shape[0]):
                ...
                for j in range(len(X_ts)):
                    x_t = X_ts[j][k]
                    x_t = (x_t + 1) * 0.5
                    utils.save_image(x_t, str(self.results_folder / f'x_{len(X_ts)-j}_{cnt}.jpg'), nrow=1)
                    x_t = cv2.imread(f'{self.results_folder}/x_{len(X_ts)-j}_{cnt}.jpg')
                    if j in to_show:
                        l.append(x_t)
    ...
    def paper_showing_diffusion_images_diff(self, s_times=None):
        ...
        for i in range(50):
            ...
            for j in range(len(X_ts_alg2)):
                x_t = X_ts_alg2[j][0]
                x_t = (x_t + 1) * 0.5
                utils.save_image(x_t, str(self.results_folder / f'x_alg2_{len(X_ts_alg2)-j}_{i}.jpg'), nrow=1)
                x_t = cv2.imread(f'{self.results_folder}/x_alg2_{len(X_ts_alg2)-j}_{i}.jpg')
                if j in to_show:
                    alg2.append(x_t)

                x_t = X_ts_alg1[j][0]
                x_t = (x_t + 1) * 0.5
                utils.save_image(x_t, str(self.results_folder / f'x_alg2_{len(X_ts_alg1) - j}_{i}.jpg'), nrow=1)
                x_t = cv2.imread(f'{self.results_folder}/x_alg2_{len(X_ts_alg1) - j}_{i}.jpg')
                if j in to_show:
                    alg1.append(x_t)
            ...
            x_0 = X_0s_alg2[-1][0]
            x_0 = (x_0 + 1) * 0.5
            utils.save_image(x_0, str(self.results_folder / f'x_best_alg2_{i}.jpg'), nrow=1)
            x_0 = cv2.imread(f'{self.results_folder}/x_best_alg2_{i}.jpg')
            alg2.append(x_0)
            im_h = cv2.hconcat(alg2)
            cv2.imwrite(f'{self.results_folder}/all_alg2_{i}.jpg', im_h)

            x_0 = X_0s_alg1[-1][0]
            x_0 = (x_0 + 1) * 0.5
            utils.save_image(x_0, str(self.results_folder / f'x_best_alg1_{i}.jpg'), nrow=1)
            x_0 = cv2.imread(f'{self.results_folder}/x_best_alg1_{i}.jpg')
            alg1.append(x_0)
            im_h = cv2.hconcat(alg1)
            cv2.imwrite(f'{self.results_folder}/all_alg1_{i}.jpg', im_h)
    ...
    def paper_showing_sampling_diff_images(self, s_times=None):
        ...
        for i in range(10):
            ...
            for j in range(og_img.shape[0]):
                utils.save_image(x0_alg1[j], str(self.results_folder / f'x0_alg1_{cnt}.jpg'), nrow=1)
                utils.save_image(x0_alg2[j], str(self.results_folder / f'x0_alg2_{cnt}.jpg'), nrow=1)
                utils.save_image(og_img[j], str(self.results_folder / f'og_img_{cnt}.jpg'), nrow=1)



                alg1 = cv2.imread(f'{self.results_folder}/x0_alg1_{cnt}.jpg')
                alg2 = cv2.imread(f'{self.results_folder}/x0_alg2_{cnt}.jpg')
                og = cv2.imread(f'{self.results_folder}/og_img_{cnt}.jpg')
    ...
    def sample_as_a_vector_gmm(self, start=0, end=1000, siz=64, ch=3, clusters=10):
        ...
        extra_path = 'vec'
        og_img = (og_img + 1) * 0.5
        utils.save_image(og_img, str(self.results_folder / f'og-{start}-{end}-{siz}-{clusters}-{extra_path}.jpg'), nrow=6)
        ...
        for i in range(len(X_0s)):
            ...
            x_0 = X_0s[i]
            x_0 = (x_0 + 1) * 0.5
            utils.save_image(x_0, str(self.results_folder / f'sample-{start}-{end}-{siz}-{clusters}-{i}-{extra_path}-x0.jpg'),
                             nrow=6)
            self.add_title(str(self.results_folder / f'sample-{start}-{end}-{siz}-{clusters}-{i}-{extra_path}-x0.jpg'), str(i))
            frames_0.append(
                imageio.imread(str(self.results_folder / f'sample-{start}-{end}-{siz}-{clusters}-{i}-{extra_path}-x0.jpg')))

            x_t = X_ts[i]
            all_images = (x_t + 1) * 0.5
            utils.save_image(all_images,
                             str(self.results_folder / f'sample-{start}-{end}-{siz}-{clusters}-{i}-{extra_path}-xt.jpg'), nrow=6)
            self.add_title(str(self.results_folder / f'sample-{start}-{end}-{siz}-{clusters}-{i}-{extra_path}-xt.jpg'), str(i))
            frames_t.append(
                imageio.imread(str(self.results_folder / f'sample-{start}-{end}-{siz}-{clusters}-{i}-{extra_path}-xt.jpg')))
    ...
    def sample_as_a_vector_gmm_and_save(self, start=0, end=1000, siz=64, ch=3, clusters=10, n_sample=10000):
        ...
        while(it):
            ...
            for i in range(x0s.shape[0]):
                utils.save_image((x0s[i]+1)*0.5, str(f'{self.results_folder}_{siz}_{clusters}/' + f'sample-x0-{cnt}.jpg'))
                cnt += 1
        ...
        while(it):
            ...
            for i in range(x0s.shape[0]):
                ...
                utils.save_image((og_img[i] + 1) * 0.5,
                                 str(f'{self.results_folder}_gmm_{siz}_{clusters}/' + f'sample-{cnt}.jpg'))

                utils.save_image((blurs[i] + 1) * 0.5,
                                 str(f'{self.results_folder}_gmm_blur_{siz}_{clusters}/' + f'sample-blur-{cnt}.jpg'))
    ...
    def sample_as_a_vector_from_blur_pytorch_gmm_and_save(self, torch_gmm, start=0, end=1000, siz=64, ch=3, clusters=10, n_sample=10000):
        ...
        while(it):
            ...
            x0s = X_0s[-1]
            for i in range(x0s.shape[0]):
                utils.save_image((x0s[i]+1)*0.5, str(f'{self.results_folder}_{siz}_{clusters}_{sample_at}/' + f'sample-x0-{cnt}.jpg'))

                utils.save_image((og_img[i] + 1) * 0.5,
                                 str(f'{self.results_folder}_gmm_{siz}_{clusters}_{sample_at}/' + f'sample-{cnt}.jpg'))
    ...
    def sample_from_data_save(self, start=0, end=1000):
        ...
        while(cnt < all_samples.shape[0]):
            ...
            x0s = X_0s[-1]
            for i in range(x0s.shape[0]):
                utils.save_image( (x0s[i]+1)*0.5, str(f'{self.results_folder}/' + f'sample-x0-{cnt}.jpg'))
                cnt += 1
    ...
    def save_training_data(self):
        ...
        for idx in range(len(dataset)):
            img = dataset[idx]
            img = (img + 1) * 0.5
            utils.save_image(img, str(f'{self.results_folder}/' + f'{idx}.jpg'))
            if idx%1000 == 0:
                print(idx)
